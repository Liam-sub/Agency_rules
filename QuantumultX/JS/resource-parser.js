...
function queryParam(raw, key) {
  var matched = String(raw).match(new RegExp("(?:^|[?&])" + key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "=([^&#;]*)"));
  return matched ? decodeURIComponent(matched[1]) : "";
}

function unsupportedTransport(type) {
  var transport = String(type || "").toLowerCase().trim();
  if (transport == "") {
    return false;
  }
  return ["grpc", "h2", "httpupgrade", "http-upgrade", "xhttp", "splithttp", "split-http", "mkcp", "kcp", "quic"].indexOf(transport) != -1;
}

function pushUnsupportedNode(raw) {
  PNS = PNS + 1;
  NSList.push(numToEmoji10(PNS) + raw);
  return "";
}
...
function VL2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  pwd = cnt.split("@")[0]? "password=" + cnt.split("@")[0]:"";
  pcert = TLSCertParam(Pcert0, cnt.indexOf("allowInsecure=0") != -1 ? "true" : "false");
  tlsHost = queryParam(cnt, "sni") ? "tls-host=" + queryParam(cnt, "sni") : ""
  tlsHost = queryParam(cnt, "peer") ? "tls-host=" + queryParam(cnt, "peer") : tlsHost
  tlsHost = queryParam(cnt, "servername") ? "tls-host=" + queryParam(cnt, "servername") : tlsHost
  tlsHost = queryParam(cnt, "serverName") ? "tls-host=" + queryParam(cnt, "serverName") : tlsHost
  ...
  if (typeU == "SR-URI") {//小火箭内的websocket写法
    tlsHost = cnt.indexOf("sni=") == -1? tlsHost : "tls-host=" + decodeURIComponent(cnt.split("sni=")[1].split("&")[0].split("#")[0]).replace(/\"|(Host\":)|\{|\}/g,"")
    tlsHost = cnt.indexOf("peer=") == -1? tlsHost : "tls-host=" + decodeURIComponent(cnt.split("peer=")[1].split("&")[0].split("#")[0]).replace(/\"|(Host\":)|\{|\}/g,"")
    ...
    if (obfs == "obfs=over-tls") {
      tlsHost = cnt.indexOf("obfsParam=") == -1? tlsHost : "tls-host=" + decodeURIComponent(cnt.split("obfsParam=")[1].split("&")[0].split("#")[0]).replace(/\"|(Host\":)|\{|\}/g,"")
    } else {
      obfsHost = cnt.indexOf("obfsParam=") == -1? obfsHost : "obfs-host=" + decodeURIComponent(cnt.split("obfsParam=")[1].split("&")[0].split("#")[0]).replace(/\"|(Host\":)|\{|\}/g,"")
    }

  puri = cnt.indexOf("path=") == -1? puri : "obfs-uri=" + decodeURIComponent(cnt.split("path=")[1].split("&")[0].split("#")[0])
  } else {
    transport = queryParam(cnt, "type")
    if (unsupportedTransport(transport)) {
      type = "NS"
    ...
    obfsHost = cnt.indexOf("&host=") == -1? obfsHost : "obfs-host=" + decodeURIComponent(cnt.split("&host=")[1].split("&")[0].split("#")[0])
    tlsHost = queryParam(cnt, "sni") ? "tls-host=" + queryParam(cnt, "sni").replace(/\"|(Host\":)|\{|\}/g,"") : tlsHost
    tlsHost = queryParam(cnt, "peer") ? "tls-host=" + queryParam(cnt, "peer").replace(/\"|(Host\":)|\{|\}/g,"") : tlsHost
    tlsHost = queryParam(cnt, "servername") ? "tls-host=" + queryParam(cnt, "servername").replace(/\"|(Host\":)|\{|\}/g,"") : tlsHost
    tlsHost = queryParam(cnt, "serverName") ? "tls-host=" + queryParam(cnt, "serverName").replace(/\"|(Host\":)|\{|\}/g,"") : tlsHost
    puri = cnt.indexOf("&path=") == -1? puri : "obfs-uri=" + decodeURIComponent(cnt.split("&path=")[1].split("&")[0].split("#")[0])
    }
  } 
...
// Reality para 2025-12-30
  ...
  nvless.push(type + ip, pwd, mtd, obfs, obfsHost, pcert, tlsHost, puri, pudp, ptfo, prlt, tag)
  QX = type!="NS"? nvless.filter(Boolean).join(", ")  : ""
  if (type=="NS") {
    pushUnsupportedNode(subs)
  }
  return QX
}
...
function TJ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
    ...
    transport = queryParam(cnt, "type")
    if (unsupportedTransport(transport)) {
      return pushUnsupportedNode(subs)
    }
    if (cnt.indexOf("&plugin=obfs-local")!=-1) {//小火箭内的websocket写法
    obfs = cnt.indexOf("obfs=websocket") != -1? "obfs=wss" : obfs 
    obfsHost=cnt.indexOf("obfs-host=") == -1? obfsHost : "obfs-host=" + decodeURIComponent(cnt.split("obfs-host=")[1].split(";")[0].split("#")[0])
    ...
    prlt= version>=891? Reality_Handle(cnt) : ""
    ntrojan.push(type + ip, pwd, obfs, obfsHost, pcert, tlsHost, puri, pudp, ptfo,prlt,tag)
...
function CV2QX(cnt) {
  ...
  tfo = cnt.tfo ? "fast-open=true" : "fast-open=false"
  if (unsupportedTransport(cnt.network)) {
    return pushUnsupportedNode(JSON.stringify(cnt))
  }
  obfs = ""
...
function CT2QX(cnt) {
  ...
  thost = cnt.servername ? "tls-host=" + cnt.servername : thost
  ...
  node = "trojan="+[ipt, pwd, otls, opath, ohost, cert, tls13, thost, udp, tfo, tag].filter(Boolean).join(", ")
  //console.log(node)
  return node
}
...
//  console.log(obfs)
  ...
  ohost = phost ? "obfs-host="+phost : ohost
  //ohost= cnt["ws-opts"]? "obfs-host=" + cnt["ws-opts"]["headers"]["Host"] : ohost
  thost = cnt["servername"]? "tls-host=" + cnt["servername"] : ""
  thost = cnt.sni ? "tls-host=" + cnt.sni : thost
  ...
  if (pspt==true) {
    pushUnsupportedNode(JSON.stringify(cnt))
    node=""
  } else {
    node = "vless="+[ipt, pwd, mtd, udp, tfo, obfs, ohost, puri, vfl, pbk, sid, cert, thost, tag].filter(Boolean).join(", ")
  }
  //console.log(node)
  return node
}
